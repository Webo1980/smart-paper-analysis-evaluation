...
import { PencilRuler, ChevronDown, ChevronUp, Info } from 'lucide-react';
...
const TitleComparison = ({ comparisonData }) => {
  const [showMetricDetails, setShowMetricDetails] = useState(false);
  
  // Calculate highlighted text differences
  const { originalHighlighted, editedHighlighted } = React.useMemo(() => 
    highlightDifferences(comparisonData.title.original, comparisonData.title.edited),
    [comparisonData]
  );

  // Extract key analysis metrics
  const titleSimilarity = comparisonData.title.levenshtein.similarityScore;
  const titleEditPercentage = comparisonData.title.edits.editPercentage;
  const levenshteinDistance = comparisonData.title.levenshtein.distance;
  const tokenF1Score = comparisonData.title.tokenMatching.f1Score;
  const precision = comparisonData.title.tokenMatching.precision;
  const recall = comparisonData.title.tokenMatching.recall;

  // Values for real-time calculations
  const calcValues = {
    distance: levenshteinDistance,
    maxLength: Math.max(
      (comparisonData.title.original || '').length, 
      (comparisonData.title.edited || '').length
    ),
    precision,
    recall,
    original: comparisonData.title.original,
    edited: comparisonData.title.edited
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center">
                ...
                <h4 className="font-medium">Edited Title</h4>
              </div>
          ...
          <div className="mt-4 p-3 bg-gray-50 rounded border text-sm">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center">
                ...
                <span className="font-medium">Title Change Analysis</span>
              </div>
              <button 
            ...
            <ul className="space-y-1 text-gray-700">
              <li>• Levenshtein Distance: {comparisonData.title.levenshtein.distance} character operations</li>
              <li>• Similarity Score: {formatPercentage(comparisonData.title.levenshtein.similarityScore)}</li>
              <li>• Token F1 Score: {formatPercentage(comparisonData.title.tokenMatching.f1Score)}</li>
              <li>• Modifications: {comparisonData.title.edits.modifications} characters changed</li>
              <li>• Length Change: {comparisonData.title.edited.length - comparisonData.title.original.length} characters ({(((comparisonData.title.edited.length / comparisonData.title.original.length) - 1) * 100).toFixed(1)}%)</li>
            </ul>
